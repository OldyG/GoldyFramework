...
public class CacheMap extends ConcurrentHashMap<String, Object> {
	...
	private static final long serialVersionUID = -7112498545450505941L;
	
	private final Map<String, Class<?>> definedKeyType = new ConcurrentHashMap<>();
	
	public void defineKey(String key, Class<?> type) {
	...
	public Object put(String key, Object value) {
		...
		if (this.definedKeyType.containsKey(key)) {
			Class<?> type = this.definedKeyType.get(key);
			
			if (type.isInstance(value) == false) {
				String message = MessageFormat.format("{0}는 [{1}] 할당으로 제한되어있습니다.", key, type.getName());
