...
public class PasswordPolicyInspection implements Inspection<String> {
	...
	private static Collection<Character> findKeyboardLine(Collection<Collection<Character>> keyboard,
		Character first) {
		
		Collection<Character> targetKeyboardLine = null;
		
		for (Collection<Character> keyboardLine : keyboard) {
	...
	private static void settingKeyboard() {
		...
		Collection<Character> keyboardLine4 = Arrays.asList('`', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0',
			'-', '=');
		Collection<Character> keyboardLine3 = Arrays.asList('q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p', '[',
			']', '\\');
		Collection<Character> keyboardLine2 = Arrays.asList('a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', ';',
			'\'');
		Collection<Character> keyboardLine1 = Arrays.asList('z', 'x', 'c', 'v', 'b', 'n', 'm', ',', '.', '/');
	...
	private static String toKeyboardCase(String target) {
	...
	protected void checkContinuedKeyboardString(String target, int limit) {
		
		SonarHelper.noStatic(this);
		String keyboardcaseTarget = toKeyboardCase(target);
		...
		for (int i = 0; i <= (keyboardcaseTarget.length() - limit); i++) {
			...
			for (int j = 0; j < (linkedList.size() - 1); j++) {
				...
				Character first = linkedList.get(j);
				Character next = linkedList.get(j + 1);
				
				List<Character> keyboardLineOfFirst = (List<Character>) findKeyboardLine(KEYBOARD, first);
				if (keyboardLineOfFirst == null) {
					break;
				}
				
				int firstIndex = keyboardLineOfFirst.indexOf(first);
				if (firstIndex > (keyboardLineOfFirst.size() - limit)) {
					break;
				}
				
				Collection<Character> keyboardLineOfNext = findKeyboardLine(KEYBOARD, next);
				if (keyboardLineOfNext == null) {
					break;
				}
				
				int nextIndex = keyboardLineOfFirst.indexOf(next);
	...
	protected void checkContinuedString(String target, int limit) {
	...
	protected void needKeyboardSpecialCharacter(String target) {
		
		SonarHelper.noStatic(this);
		Collection<Character> specialList = Arrays.asList(
			'!', '@', '#', '$',
			'%', '^', '&', '*',
			'(', ')', '-', '_',
			'=', '+', '[', '{',
		...
		throw new InspectionException("특수문자를 1개 이상 입력해야합니다.");
	}
}
