...
public class WhereGuideTest extends Mockito {
	...
	/**
	 * Test method for
	 * {@link com.goldyframework.db.prepare.statement.guide.WhereGuide#append(java.lang.String, int)}.
	 */
	@Test
	public void testAppend() {
		
		WhereGuide target = new WhereGuide("TEST");
		target.append("ColumnName1", Comparison.EQUAL, "123");
		target.append("ColumnName2", Comparison.EQUAL, 11_455);
		
		Map<String, ComparisonValue> whereMap = target.getCopiedWhereMap();
	...
	public void testBuild() {
		...
		String actualEmpty = target.toSql();
		Assert.assertTrue("", actualEmpty.isEmpty());
		
		target.append("ColumnName2", Comparison.EQUAL, 11_455);
		target.append("ColumnName1", Comparison.EQUAL, "123");
		
		String actual2 = target.toSql();
		Assert.assertFalse("", actual2.isEmpty());
		
		Object next = target.getArgs().iterator().next();
	...
	/**
	 * Test method for
	 * {@link com.goldyframework.db.prepare.statement.guide.WhereGuide#eachAppendComparisonValue()}.
	 */
	@Test
	@UnitTest
	public void testEachAppendComparisonValue() {
		
		WhereGuide target = spy(new WhereGuide("TEST"));
		
		// 내부 변수
		Map<String, ComparisonValue> copiedWhereMap = spy(new ConcurrentHashMap<>());
		...
		// 실행
		List<String> actual = target.eachAppendComparisonValue();
	...
	public void testGetArgs() {
		...
		Collection<Object> args = target.getArgs();
