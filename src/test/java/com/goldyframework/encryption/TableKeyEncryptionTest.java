...
public class TableKeyEncryptionTest {
	...
	public void testEncrypt() throws EncryptionException {
		...
		final TableKeyEncryption target = new TableKeyEncryption();

		final String encrypt1 = target.encrypt(3);
		final String encrypt2 = target.encrypt(3);

		Assert.assertNotEquals(encrypt1, encrypt2);

		final int decrypt1 = target.decrypt(encrypt1);
		final int decrypt2 = target.decrypt(encrypt2);
